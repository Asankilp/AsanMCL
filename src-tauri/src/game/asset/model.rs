...
mod tests {
    ...
    fn test_parse_asset_index() {
        ...
        for i in asset_index_files.iter() {
            ...
            let asset_index: GameAssetIndex = serde_json::from_str(&content).expect("Failed to parse asset index JSON");
            assert!(!asset_index.objects.is_empty(), "Asset index should not be empty");
